...
0. **Innovation gate**: If the experiment only changes a numeric hyperparameter (LR, weight decay, batch size, threshold, epoch count, schedule) without modifying model/loss/algorithm code, reject it with `review_reason: "hyperparameter-only change is not a research contribution"` — unless it is explicitly for baseline anchor reproduction or ablation support for a method experiment.

...
CONFIG="${CONFIG:-configs/unbiased_teacher/sfod/unbiased_teacher_oriented_rcnn_selftraining_cga.py}"
DATA_ROOT="${DATA_ROOT:-dataset/DIOR}"
WORK_DIR="${WORK_DIR:-work_dirs/exp_dior_ut_cga_clip}"
SPLIT_DIR="${SPLIT_DIR:-work_dirs/smoke_splits/dior_ut_cga_clip}"
...
export CGA_SCORER="${CGA_SCORER:-clip}"
if [[ -z "${CGA_TEMPLATES:-}" ]]; then
  export CGA_TEMPLATES='an aerial image of a {}'
fi
...
echo "[exp_dior_ut_cga_clip] train corrupt=clean (ENV=${ENV_NAME}, CGA_SCORER=${CGA_SCORER}) ..."
...
for c in "${CORRUPT_LIST[@]}"; do
  ...
  echo "[exp_dior_ut_cga_clip] test corrupt=${c} (ENV=${ENV_NAME}) ..."
...
echo "[exp_dior_ut_cga_clip] done: ${WORK_DIR}"

#!/bin/bash
# 7 种干扰 × OrthoNet + SARCLIP RN50 LoRA + 12ep SFOD
# 10 GPU + DiskCache (RN50 比 ViT-L-14 快 8x, 第2 epoch 后全命中缓存)
set -e
...
# 用 RN50 替代 ViT-L-14 (快 8 倍, 显存少 10 倍)
export SARCLIP_MODEL=RN50
export SARCLIP_PRETRAINED="/mnt/SSD1_8TB/zechuan/IRAOD/weights/sarclip/RN50/rn50_model.safetensors"
...
echo "============================================================"
echo "  7 Corruptions × OrthoNet + SARCLIP RN50 + 12ep SFOD"
echo "  10 GPU + DiskCache"
echo "  SARCLIP_MODEL=RN50 (8x faster than ViT-L-14)"
echo "  Start: $(date)"
echo "============================================================"
...
for corrupt in "${CORRUPTS[@]}"; do
    ...
    cache_dir="work_dirs/cga_cache_rn50_${corrupt}"
    rm -rf "$work_dir"
    ...
    echo ""
    echo "[$(date +%H:%M:%S)] === $corrupt === (10 GPU, RN50, cache=$cache_dir)"

...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Detection-approx SHOT baseline on RSAR/train.")
    ...
    parser.add_argument("--epochs", type=int, default=3, help="Number of adaptation epochs.")
    parser.add_argument("--max-steps", type=int, default=0, help="Optional cap on optimizer steps (0 = full loader).")
    parser.add_argument("--lr", type=float, default=1e-4, help="SHOT optimizer learning rate.")
...
def main() -> None:
    ...
    trainable_params, trainable_names = configure_shot_trainable(model)
    if not trainable_params:
        raise RuntimeError("SHOT found no trainable backbone/neck parameters.")
    ...
    method_dir = Path(args.result_root) / "shot"
    ...
    for _epoch in range(args.epochs):
        for data in loader:
            ...
            if step % 20 == 0:
                print(
                    f"[run_shot_adapt] step={step} entropy={stats['entropy']:.4f} "
    ...
    shot_ckpt = method_dir / "latest.pth"
    save_model_checkpoint(
        model,
        shot_ckpt,
        meta={
            ...
            "definition": "detection-approx-shot: freeze RPN/RoI heads, optimize backbone+neck with RoI entropy on target train.",
        },
    )
    json_dump(
    ...
    print(f"[run_shot_adapt] steps={step}")
    print(f"[run_shot_adapt] mean={payload['metrics']['mean']:.4f}")
    print(f"[run_shot_adapt] wrote {csv_path}")
    print(f"[run_shot_adapt] wrote {md_path}")

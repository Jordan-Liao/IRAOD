...
if use_tta:
    _tta_scales = [(800, 800), (1024, 1024)]
    test_pipeline = [
        dict(type="LoadImageFromFile"),
        dict(
            type="MultiScaleFlipAug",
            img_scale=_tta_scales,
            flip=True,
            flip_direction=["horizontal"],
            transforms=[
                dict(type="RResize"),
                dict(type="RRandomFlip", version=angle_version),
                dict(type="Normalize", **img_norm_cfg),
                dict(type="Pad", size_divisor=32),
                dict(type="DefaultFormatBundle"),
...
if stage == "source_train":
    ...
    burn_in_epochs = int(os.environ.get("RSAR_BURN_IN_EPOCHS", "0").strip() or "0")
    weight_u = float(os.environ.get("RSAR_WEIGHT_U", "1.0").strip() or "1.0")
    _stre = os.environ.get("RSAR_PSEUDO_SCORE_THR", "0.7").strip()
    if "," in _stre:
    ...
    _adapt_lr = os.environ.get("RSAR_ADAPT_LR", "").strip()
    optimizer = dict(type="SGD", lr=float(_adapt_lr) if _adapt_lr else 0.02, momentum=0.9, weight_decay=0.0001)

# ============================================================================
# Corrected semi-supervised config for RSAR
# Key fixes vs original (unbiased_teacher_oriented_rcnn_selftraining_cga_o_rsar.py):
#   1. Student backbone: OrthoNet -> ResNet-50 (matches Teacher, eliminates mismatch)
#   2. load_from + ema_ckpt: baseline checkpoint (mAP 0.653)
#   3. burn-in: first 2 epochs weight_u=0 (avoid early pseudo-label noise)
#   4. score_thr: 0.7 -> 0.5 (more pseudo labels)
#   5. EMA: cosine schedule 0.996 -> 0.9999
#   6. LR: 0.02 -> 0.005 (fine-tuning from pretrained ckpt)
#   7. LR schedule: step [8, 11] aligned with baseline
# ============================================================================
...
# ------------------ 基本训练超参 ------------------
gpu = 1
# Exp E: per-class thresholds (ship, aircraft, car, tank, bridge, harbor)
score = [0.5, 0.5, 0.5, 0.8, 0.7, 0.6]  # per-class thresholds
samples_per_gpu = 2
total_epoch = 12
test_interval = 1
save_interval = 1
...
# P0-1: 从预训练 checkpoint 微调，LR 降至 0.005
...
# LR schedule: step [8, 11] 与 baseline 对齐
...
model = dict(
    ...
    cfg=dict(
        ...
        weight_u=1,
        debug=False,
        score_thr=score,            # per-class: [0.5,0.5,0.5,0.8,0.7,0.6]
        use_bbox_reg=True,
        momentum=0.996,             # P1-2: cosine schedule 起始值
        # --- 新增：burn-in ---
        burn_in_epochs=2,           # P0-2: 前 2 epoch 不使用伪标签
        # --- 新增：cosine EMA schedule ---
        ema_schedule='cosine',
        ema_momentum_end=0.9999,    # cosine 终止值
        max_iters=47304,            # 12 epochs * ~3942 iters/epoch
    ),
    # P0-1: Student backbone 改为 ResNet-50（与 Teacher 一致）
    backbone=dict(
        type='ResNet',
        depth=50,
        num_stages=4,
